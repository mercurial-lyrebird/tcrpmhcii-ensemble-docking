...
if __name__ == "__main__":
    ...
    angle_max = int(sys.argv[2] if len(sys.argv) > 2 else 360)

    dest_dir = os.path.join("./data/mhcii_ensembles/", pdb_id, f"basinhop_max={angle_max}")
    ...
    left_minimizer = MinMover(left_move_map, fxn, "dfpmin", 0.01, True)
    ...
    right_minimizer = MinMover(right_move_map, fxn, "dfpmin", 0.01, True)
    ...
    for i in tqdm(range(100)):
        ...
        if os.path.exists(os.path.join(dest_dir, f"{job_id}_bh{angle_max}_{i}.pdb")):

            q_p = pose_from_pdb(os.path.join(dest_dir, f"{job_id}_bh{angle_max}_{i}.pdb"))
            qs.append((q_p, fxn(q_p), i))
            ...
            q.dump_pdb(os.path.join(dest_dir, f"{job_id}_bh{angle_max}_{i}.pdb"))
            qs.append((deep_copy(q), fxn(q), i))

    n_iter = int(sys.argv[3] if len(sys.argv) > 3 else 1000)

    dest_dir = os.path.join("./data/mhcii_ensembles/", pdb_id, f"basinhop_annealing_max={angle_max}_iter={n_iter}")
    ...
    def anneal(arg):
        ...
        (q_p, init_score, i) = arg

        mc = MonteCarlo(q_p, fxn, init_temp)

        for j in tqdm(range(n_iter)):

            q_p.fold_tree(left_ft)
            left_mover.apply(q_p)

            q_p.fold_tree(right_ft)
            right_mover.apply(q_p)
        ...
        mc.recover_low(q_p)
        print(f"{init_score} -> {fxn(q_p)}")

        q_p.dump_pdb(os.path.join(dest_dir, f"{job_id}_{ts}_{i}.pdb"))
